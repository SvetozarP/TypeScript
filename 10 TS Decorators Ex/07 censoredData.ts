...
function getLatest(seconds: number) {
    return function(target: any, methodName: string, descriptor: PropertyDescriptor) {
        ...
        descriptor.value = function() {
            ...
            const recentItems = allItems.filter((items: any) => currentTime.getTime() - items.createdOn.getTime() <= seconds * 1000);
            return recentItems;
        }
...
function censorProps<T extends { [key: string]: any }>(service: MockCensorService<T>) {
    return function (target: any, methodName: string, descriptor: PropertyDescriptor) {
        const originalMethod = descriptor.value;

        descriptor.value = function() {
            const allItems = originalMethod.call(this);
            const censoredItems = service.censorProperties(allItems);
            return censoredItems;
        }
